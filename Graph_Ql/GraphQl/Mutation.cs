...
namespace Graph_Ql.GraphQl
{
    public class Mutation
    {
        ...
        public async Task<Employee> CreateEmployeeWithDepartmentIdByUsingObject([Service] EmployeeRepository employeeRepository,
           Employee employee)
        {
            var createdEmployee = await employeeRepository.CreateEmployee(employee);
            return createdEmployee;
        }
        ...
        public async Task<Employee> UpdateEmployee([Service] EmployeeRepository employeeRepository,
         int employeeId, string name, int age, string email)
        {
            var UpdateEmployee = await employeeRepository.UpdateEmployee(employeeId, name, age, email);
            return UpdateEmployee;
        }

        public async Task<bool> DeleteOneEmployee([Service] EmployeeRepository employeeRepository,
          int employeeId)
        {
            Employee DeletedEmp = await employeeRepository.GetEmployeeById(employeeId);

            await employeeRepository.DeleteEmployee(DeletedEmp);
            return true;
        }
    }
}
